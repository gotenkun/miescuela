...
public partial class admin_Dejar_Tarea_Individual : System.Web.UI.Page
{
    ...
    private void Cargar()
    {
        try
        {
            using (MiEscuelaDataContext context = new MiEscuelaDataContext())
            {
                ...
                {
                    var usuario = context.Usuarios.Where(p => p.UsuarioID == long.Parse(Request.GetFriendlyUrlSegments()[0])).SingleOrDefault();
                    if (usuario != null)
                    {
                        this.lblGrupo.Text = usuario.Nombre + " " + usuario.Apellidos;
                        // lblTitulo.Text = area.Nombre;
                        lblTitulo1.Text = lblGrupo.Text;
                        hdID.Value = usuario.UsuarioID.ToString();
                        hlCancelar.HRef = "/Admin/Grupo/" + usuario.UsuarioGrupos.First().GrupoID;
                        lblNumeroAlumnos.Text = "Se enviará esta tarea a " + lblGrupo.Text;
                    }
                }
            }
        }
    ...
    protected void btnGuardar_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            using (MiEscuelaDataContext context = new MiEscuelaDataContext())
            {
                ...
                i.UsuarioID = long.Parse(hdID.Value);
